...
//    class PNG
      ...
      class PNGUpsilon extends PNG{
        ...
        PNGUpsilon(UpsilonMaze maze,PNGFormatterOptions options):super(maze, options){
          ...
          Metrics metrics = new Metrics(size: options.cell_size - options.cell_padding * 2 );
        ...
        var px = options.outer_padding + x * metrics.inc;
              ...
              if ((y + x) % 2 == 0){
              _draw_octogon_cell(canvas, new Position.xy(x, y), px, py, cell, metrics);
              }else {
                _draw_square_cell(canvas, new Position.xy(x, y), px, py, cell, metrics);
              }
            }
          }
...
//        any = proc { |x| x | (x << Maze.UNDER_SHIFT) };
        ...
        _draw_octogon_cell(PNGCanvas canvas,Position point,num x,num y,int cell,Metrics metrics){ //#:nodoc:
          var p1 = new Position.xy(x + options.cell_padding + metrics.s4, y + options.cell_padding);
          var p2 = new Position.xy(x + options.cell_size - options.cell_padding - metrics.s4, p1.y);
          var p3 = new Position.xy(x + options.cell_size - options.cell_padding, y + options.cell_padding + metrics.s4);
          var p4 = new Position.xy(p3.x, y + options.cell_size - options.cell_padding - metrics.s4);
          var p5 = new Position.xy(p2.x, y + options.cell_size - options.cell_padding);
          ...
          if (cell & any(Maze.E) != 0){
            var edge = (x + options.cell_size + options.cell_padding > canvas.width);
            var r1 = p3;
            var r2 = edge ? move(p4, options.cell_padding, 0) : move(p7, options.cell_size, 0);
...
if (cell & Maze.NW == 0){  _line(canvas, p8, p1, options.wall_color) ;}
        ...
        _draw_square_cell(PNGCanvas canvas,Position point,num x,num y,int cell,Metrics metrics) {//#:nodoc:
          var v = options.cell_padding + metrics.s4;
          var p1 = new Position.xy(x + v, y + v);
          var p2 = new Position.xy(x + options.cell_size - v, y + options.cell_size - v);
...
if (cell & Maze.N == 0){ _line(canvas, p1, new Position.xy(p2.x, p1.y), options.wall_color);};
if (cell & Maze.E == 0){      _line(canvas, new Position.xy(p2.x, p1.y), p2, options.wall_color) ;}
if (cell & Maze.S == 0){       _line(canvas, new Position.xy(p1.x, p2.y), p2, options.wall_color);}
if (cell & Maze.W == 0){     _line(canvas, p1, new Position.xy(p1.x, p2.y), options.wall_color);}
        }
      }
