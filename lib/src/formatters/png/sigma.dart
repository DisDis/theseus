              ...
              for (int y = 0; y < maze.height; y++) {
                  ...
                  for (int x = 0; x < maze.row_length(y); x++) {
                      var px = options.outer_padding +
                          x * 3 * options.cell_size / 4.0;
                      var shifted = (x % 2 != 0);
                      var dy = shifted ? (options.cell_size / 2.0) : 0;
                      _draw_cell(
                          canvas, new Position.xy(x, y), shifted, px, py + dy,
                          maze.getCell(x, y));
                  }
              }
          }
...
// any = proc { |x| x | (x << Maze.UNDER_SHIFT) };
          ...
          void _draw_cell(PNGCanvas canvas, Position point, bool shifted, num x,
              ...
              var p2 = new Position.xy(
                  x + options.cell_size - options.cell_padding - s4, p1.y);
...
//          any = proc { |x| x | (x << Maze.UNDER_SHIFT) };
              ...
              if (cell & any(ne) != 0) {
                  ...
                  var r4 = ne_p6;
                  _fill_poly(
                      canvas, [r1, r2, r3, r4], color_at(point, any(ne)));
...
//            r1, r2, r3, r4 = p3, p4, se_p6, se_p1;
                  ...
                  var r4 = se_p1;
                  _fill_poly(
                      canvas, [r1, r2, r3, r4], color_at(point, any(se)));
                  _line(canvas, r1, r4, options.wall_color);
                  _line(canvas, r2, r3, options.wall_color);
              }
