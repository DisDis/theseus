...
//    class PNG
      ...
      class PNGOrthogonal extends PNG{
        ...
        PNGOrthogonal(OrthogonalMaze maze,PNGFormatterOptions options):super(maze, options){
         ...
         // canvas = ChunkyPNG::Image.new(width, height, options.background);
          ...
          for (int y = 0 ; y < maze.height; y++){//maze.height.times do |y|
            ...
            for (int x = 0 ; x < maze.width; x++){//maze.width.times do |x|
        ...
        _draw_cell(canvas, Position point, x, y, cell) {//#:nodoc:
          if (cell == 0) {
              return;
          }
          ...
          _draw_horizontal(canvas, x, y, 1, west || west_under, !west || west_under, color_at(point, ANY_W));
          _draw_horizontal(canvas, x + options.cell_size, y, -1, east || east_under, !east || east_under, color_at(point, ANY_E));
        }

        _draw_vertical(canvas, x, y, direction, corridor, wall, color){ //#:nodoc:
          if (corridor){
            ...
            _fill_rect(canvas, x + d2 - w2, y - (w1 * direction), x + d2 + w1, y + (d1 + w2) * direction, options.wall_color);
          }

          if (wall){
            _fill_rect(canvas, x + d1 - w1, y + (d1 - w1) * direction, x + d2 + w2, y + (d1 + w2) * direction, options.wall_color);
          }
        }

        _draw_horizontal(canvas, x, y, direction, corridor, wall, color){ //#:nodoc:
          if (corridor){
            ...
            _fill_rect(canvas, x - (w1 * direction), y + d2 - w2, x + (d1 + w2) * direction, y + d2 + w1, options.wall_color);
          }

          if (wall){
            _fill_rect(canvas, x + (d1 - w1) * direction, y + d1 - w1, x + (d1 + w2) * direction, y + d2 + w2, options.wall_color);
          }
        }
      }
