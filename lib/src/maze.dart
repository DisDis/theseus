...
class MazeOptions {
  ...
  MazeOptions({
    this.algorithm = RecursiveBacktracker,
    this.braid = 0,
    Position? entrance,
    Position? exit,
    this.height = 10,
    this.width = 10,
    this.mask = const TransparentMask(),
    this.prebuilt = false,
    this.randomness = 100,
    this.symmetry,
    this.weave = 0,
    this.wrap,
  })  : entrance = entrance ?? Position.xy(-1, 0),
        exit = exit ?? Position.xy(width, height - 1);
}
...
abstract class Maze {
  ...
  late Base /*attr_reader :*/ _algorithm;
...
//   }
  ...
  Maze(MazeOptions options/*={}*/)
      : _width = options.width,
        ...
        _braid = options.braid,
        _symmetry = options.symmetry,
        _randomness = options.randomness,
        _mask = options.mask,
        _weave = options.weave,
        _wrap = options.wrap {
    _configure_symmetry();
    _cells = _setup_grid();

    _entrance = options.entrance ?? _default_entrance();
    _exit = options.exit ?? _default_exit();

    _algorithm = resolveAlgorithms(options.algorithm, this, options);
...
//         return new klass(self, a, b);
  ...
  List<Position> dead_ends() {
    ...
    ruby.each_with_index<List<int>>(_cells,
  ...
  bool dead /*?*/ (int cell) {
    ...
    return raw == N || raw == S || raw == E || raw == W || raw == NE || raw == NW || raw == SE || raw == SW;
  }
  ...
  static final int? direction_under = null;
...
//        generated? ? "generated" : "not generated"]
  ...
  bool weave_allowed /*?*/ (int from_x, int? from_y, int thru_x, int thru_y, int direction) {
    //#:nodoc:
  ...
  List<int> perform_weave(int from_x, int from_y, int to_x, int to_y, int direction) {
    //#:nodoc:
    if (ruby.rand(2) == 0) {
  ...
  Position _default_entrance() {
    ...
    ruby.each_with_index<List<int>>(_cells, (row, y) {
      //_cells.each_with_index do |row, y|
      return ruby.each_with_index<int>(row, (cell, x) {
        //row.each_with_index do |cell, x|
  ...
  Position _default_exit() {
    ...
    ruby.each_with_index<List<int>>(_cells.reversed, (List<int> row, y) {
      ...
      return ruby.each_with_index<int>(row.reversed, (cell, x) {
